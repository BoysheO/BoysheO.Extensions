...
namespace System.Collections.Generic
{
    ...
    public sealed partial class ArraySortHelper<T>
    {
        ...
        internal static int InternalBinarySearch(T[] array, int index, int length, T value, IComparer<T> comparer)
        {
            ...
            Debug.Assert(index >= 0 && length >= 0 && (array.Length - index >= length),
        ...
        private static void IntroSort(Span<T> keys, int depthLimit, Comparison<T> comparer)
        {
            ...
            {
                if (partitionSize <= Const.IntrosortSizeThreshold)
                {
                    if (partitionSize == 2)
                    {
        ...
        where T : IComparable<T>
    {
        ...
        {
            Debug.Assert(keys.Length >= Const.IntrosortSizeThreshold);
...
#pragma warning restore 8500
    ...
    internal sealed partial class ArraySortHelper<TKey, TValue>
    {
        ...
        private static void SwapIfGreaterWithValues(Span<TKey> keys, Span<TValue> values, IComparer<TKey> comparer,
            int i, int j)
        {
            Debug.Assert(comparer != null);
